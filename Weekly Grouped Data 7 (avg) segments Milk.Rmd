...
```{r}
df <- read.csv("df_Milk_full_tax_difference.csv")
...
df_long <- df_filtered %>%
  pivot_longer(cols = c(full_tax_count, milk_count),
               names_to = "control",
               values_to = "count") %>%
...
```{r}
acf(df_regression$count, lag.max = 365,main = "ACF of count")
...
####Newey West Method
```{r}
NW <- NeweyWest(seg_model,lag=250,prewhite =F,adjust=T)
NW_se <- sqrt(diag(NW))
print(NW_se)
